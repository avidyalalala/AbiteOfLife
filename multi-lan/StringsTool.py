...
class SourceLoader:
	...
	def saveToExcel(self):
		...
		file.save(self.path + '/strings.xls')
		print 'done! export to ' + self.path + '/strings.xls'
		#if len(l) == 0:
...
'''
class TargetCreator:
	def __init__(self, path = '.', filename = 'strings.xls'):
	...
	def loadFromExcel(self,targetLang):
		...
		data = xlrd.open_workbook(self.path + '/' + self.filename)
		table = data.sheets()[0]
...
def  doImport(path, lang):
	creator = TargetCreator(path)
