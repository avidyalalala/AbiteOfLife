...
def downloadFile(index_dir,hour):
    ...
    print("%s create a %s size thread Pool to handle %s files about the hostname split and dispatch"%(str(os.getpid()),str(size),str(len(downloadList))))

    output_dir=index_dir
    threadPool.map_async(handler,itertools.izip(downloadList, itertools.repeat(output_dir,len(downloadList))))
    threadPool.close()
    threadPool.join()
    print("threads pool are closed at: %s by %s"%(str(datetime.now())),str(os.getpid()))
    #print(the_page)
    return;
